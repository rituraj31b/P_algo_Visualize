...
function ListBlocks({ blocks, compare, sorted, swap }){
    const [width, setWidth] = useState(Math.min(200, Math.ceil(window.innerWidth / blocks.length) - 5))
    const color = blocks.length <= 50 && width > 14 ? 'white' : 'transparent'

    useEffect(() => {
        const handleResize = () => {
            setWidth(Math.min(200, Math.ceil(window.innerWidth / blocks.length) - 8))
        }
